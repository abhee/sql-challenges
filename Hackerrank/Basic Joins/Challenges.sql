...
Having TotalChallenges in
       (Select TotalChallenges
        From (Select Hacker_Id, count(*) As TotalChallenges From Challenges Group By Hacker_Id) As T
        Group By TotalChallenges
        Having (TotalChallenges = (Select MAX(Cnt)
                                   From (Select count(*) As Cnt From Challenges Group By Hacker_Id) A))
            OR (TotalChallenges < (Select MAX(Cnt)
                                   From (Select count(*) As Cnt From Challenges Group By Hacker_Id) A)
